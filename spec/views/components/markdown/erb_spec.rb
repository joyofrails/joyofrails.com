...
RSpec.describe "Markdown with Erb", type: :view do
  ...
  it "ignores inline fenced erb following text" do
    expect(render_component("This is the number <%= 1 + 1 %>").strip).to eq("<p>This is the number <%= 1 + 1 %></p>")
  end

  it "escapes inline fenced erb" do
    expect(render_component("`<%= 1 + 1 %>`").strip).to eq("<p><code>&lt;%= 1 + 1 %&gt;</code></p>")
  end
  ...
  it "handles fenced erb with text" do
    ...
    expect(render_component(md)).to include("color_name = @color_scheme.name.parameterize")
  end
end
